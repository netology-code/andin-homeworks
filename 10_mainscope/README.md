...
1. Откройте ваш проект Android-приложения с предыдущего ДЗ (можно взять код с лекции).
...
Для реализации второго пункта внесите в `PostEntity` дополнительные поля, отвечающие за данный статус. На базе них можно рисовать иконку статуса в карточке поста + продумывать механику взаимодействия: например, несохранённый пост нельзя лайкнуть.
</details>

Не забудьте про `Retry` для повторной попытки вызова API сохранения, если возникнут проблемы. Например, из-за отсутствия сетевого подключения.
